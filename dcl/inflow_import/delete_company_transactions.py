...
from frappe.desk.notifications import clear_notifications,clear_doctype_notifications

import functools

#dcl.inflow_import.delete_company_transactions.delete_company_transactions  --kwargs "{'filename':'inFlow_PurchaseOrder_199.csv'}"
@frappe.whitelist()
def delete_company_transactions(file):
    company_name = "DCL Laboratory Products Limited"
    ...
    for doctype in frappe.db.sql_list("""select parent from
        ...
        if doctype in ("Sales Order",
                       ...
                       "Purchase Receipt",
                       "Quotation"):
            clear_doctype_notifications(doctype)
            delete_for_doctype(doctype, company_name,file)
...
def delete_for_doctype(doctype, company_name,file):
        ...
        if not meta.istable:
            ...
            for df in meta.get_table_fields():
                frappe.db.sql("""delete from `tab{0}` where parent in
					(select name from `tab{1}` where `{2}`=%s and inflow_file=%s)""".format(df.options,
                                                                         doctype, company_fieldname), (company_name,file))

        # delete version log
        frappe.db.sql("""delete from `tabVersion` where ref_doctype=%s and docname in
			(select name from `tab{0}` where `{1}`=%s and inflow_file=%s)""".format(doctype,
                                                                 company_fieldname), (doctype, company_name,file))


        parents = frappe.db.sql("""select name from `tab{0}`
        			where {1}= %s and inflow_file=%s """.format(doctype, company_fieldname), (company_name, file))
        # delete gl entry
        for p in parents:
            frappe.db.sql("""delete from `tabGL Entry`
            where {0}= %s and voucher_no=%s """.format(company_fieldname), (company_name,p[0]))

        # delete stock gl entry
        for p in parents:
...
def delete_lead_addresses(company_name):
    ...
    if leads:
        ...
        frappe.db.sql(
            """update tabCustomer set lead_name=NULL where lead_name in ({leads})""".format(leads=",".join(leads)))
