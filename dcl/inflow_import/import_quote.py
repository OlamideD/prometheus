...
#             payment_request = make_payment_request(dt="Sales Order", dn=so.name, recipient_id="",
...
def start_import(file):
    ...
    for i,row in enumerate(rows):
        ...
        #     continue
        #
        skip_rows = ["QTN-000741","QTN-000771","QTN-001095","QTN-001172"]
        ...
        if current_customer != row["Customer"].strip() or current_customer != row["Customer"].strip() \
            ...
            if totalrows == i and current_customer == row["Customer"]:
                ...
                last_single_SI_dict = {"doctype": "Quotation",
                           "title": current_customer,
                           "customer": current_customer,
                           "posting_date": dt.date(),
                           "schedule_date": dt.date(),  # TODO + 30 days
            ...
            rename_doc("Quotation",SI_created.name,current_order,force=True)
            frappe.db.commit()
    ...
    if last_single_SI_dict != {}:
        ...
        #     make_invoice(last_single_paid_items,current_customer,SI_dict)
        #
        # if last_single_fulfilled_items:
        #     make_delivery(fulfilled_items,current_order,last_single_fulfilled_items,to_warehouse)
...
def remove_imported_data(file,force=0):
    if force == 1:
        SIs = frappe.db.sql("""SELECT name FROM `tabQuotation`""")
    else:
        SIs = frappe.db.sql("""SELECT name FROM `tabQuotation` WHERE inflow_file=%s""",(file))

    for si in SIs:
        si_doc = frappe.get_doc("Quotation", si[0])
